...
from eval.cider.cider_NOND import Cider_NOND
from eval.cider.cider_R import Cider_R
from eval.cider.cider_R_author import Cider_R_author
from eval.spice.spice import Spice


"""
I do not own the rights of this code, I just modified it according to my needs.
The original version can be found in: https://github.com/cocodataset/cocoapi
"""
class COCOEvalCap:
    ...
    def evaluate(self, bleu=True, rouge=True, cider=True, spice=True, meteor=True, verbose=True):
        ...
        if cider:
            ...
            #scorers.append((Cider_R_author(), "CIDEr_R_author"))
            #scorers.append((Cider_NOND(), "CIDEr-NOND"))
            #scorers.append((Cider_R(), "CIDEr-R"))
        if bleu:
        ...
        for scorer, method in scorers:
            ...
            if type(method) == list:
                ...
                return_scores.append((method, score))
        self.setEvalImgs()
